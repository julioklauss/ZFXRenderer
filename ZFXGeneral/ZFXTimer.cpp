...
* Constructor
*/
ZFXTimer::ZFXTimer(UCHAR nHH, UCHAR nMM)
{
	m_fClock		= 0.0f;
	m_nCur_time		= 0;
	m_nLast_time	= 0;
	...
	if (QueryPerformanceCounter((LARGE_INTEGER*)&m_nPerf_cnt)) {
...
void ZFXTimer::Update()
{
	...
	m_fTime_elapsed = (m_nCur_time - m_nLast_time) * m_fTime_elapsed;
...
*/
void ZFXTimer::SetClock(UCHAR nHH, UCHAR nMM)
{
	// set clock
	if (nHH > 23)
		nHH = 0;
...
/**
* Gives you a times tring with hours, minutes and seconds as return
* and hours and/pr minutes as reference parameters.
*/
TCHAR* ZFXTimer::GetClock(UCHAR* nHH, UCHAR* nMM, TCHAR* pChar)
{
	if (nHH != NULL)
		*nHH = m_nHH;
	if (nMM != NULL)
	...
	swprintf_s(pChar, sizeof(TCHAR), L"%.2d:%.2d:%.2d", m_nHH, m_nMM, m_nSS);
	return pChar;
}	//	GetClock
/*----------------------------------------------------------------*/