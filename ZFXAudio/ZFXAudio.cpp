// ZFXAudio.cpp : Defines the functions for the static library.
//
...
{
	// load the DLL containing the interface implementation
	m_hDLL = LoadLibraryExA("ZFXDA.dll", NULL, 0);
	if (!m_hDLL) {
		MessageBoxA(NULL, "Loading ZFXDA.dll from lib failed.", "ZFXEngine - error", MB_OK | MB_ICONERROR);
		return E_FAIL;
	}
	...
	_CreateAudioDevice = (CREATEAUDIODEVICE)GetProcAddress(m_hDLL, "CreateAudioDevice");
	// call dll's create function
	hr = _CreateAudioDevice(m_hDLL, &m_pDevice);
	if (FAILED(hr)) {
		MessageBoxA(NULL, "CreateAudioDevice() from lib failed.", "ZFXEngine - error", MB_OK | MB_ICONERROR);
		m_pDevice = NULL;
		return E_FAIL;
	}
...
void ZFXAudio::Release(void)
{
	...
	if (m_hDLL) {
		...
		_ReleaseAudioDevice = (RELEASEAUDIODEVICE)GetProcAddress(m_hDLL, "ReleaseAudioDevice");
	}
