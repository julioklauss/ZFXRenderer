...
*/
HRESULT ZFXD3DEnum::Enum(HWND hAdapter, HWND hMode, HWND hDevice, HWND hAdapterFmt, HWND hBackFmt, HWND hWnd, HWND hFull, FILE* pLog)
{
	HRESULT hr;
	...
	// Create a temp variable so use tchar instead of char
	...
	for (UINT a = 0; a < m_dwNumAdapters; ++a)
	{
		...
		AddItem(m_hADAPTER, m_xAdapterInfo[a].d3dAdapterIdentifier.Description, &m_xAdapterInfo[a]);
	}
...
void ZFXD3DEnum::AddItem(HWND hWnd, const char* ch, void* pData)
{
...
bool ZFXD3DEnum::ContainsString(HWND hWnd, const char* ch)
{
...
*/
void ZFXD3DEnum::GetSelections(ZFXDEVICEINFO* pD, D3DDISPLAYMODE* dspmd, D3DFORMAT* fmtA, D3DFORMAT* fmtB)
{
	if (pD)
...
void ZFXD3DEnum::ChangedAdapter(void)
{
	...
	{
		AddItem(m_hDEVICE, D3DDevTypeToString(pA->d3dDevs[d].d3dDevType), &pA->d3dDevs[d]);
	}
	SendMessage(m_hDEVICE, CB_SETCURSEL, (WPARAM)0, 0);
...
void ZFXD3DEnum::ChangedDevice()
{
	...
	// active radio buttons accordingly
...
/*
*	Call when window/fullscreen selection changed. This will relist adapter-
...
void ZFXD3DEnum::ChangedWindowMode(void)
{
	...
	if (SendMessage(m_hWND, BM_GETCHECK, 0, 0) == BST_CHECKED)
	{
		...
		AddItem(m_hADAPTERFMT, D3DFormatToString(m_dspmd.Format), &m_dspmd.Format);
		// user current desktop resolution
		sprintf_s(buffer, "%d x %d", m_dspmd.Width, m_dspmd.Height);
		...
		for (UINT c = 0; c < pD->nNumCombo; ++c)
		{
			...
			if (!ContainsString(m_hADAPTERFMT, D3DFormatToString(pC->fmtAdapter)))
				AddItem(m_hADAPTERFMT, D3DFormatToString(pC->fmtAdapter), &pC->fmtAdapter);
		}	// for [combos]
	}
...
/*
*	Call when adapter format selection changed. Will reslist mode and
...
void ZFXD3DEnum::ChangedAdapterFmt(void)
{
	...
	if (!bWindowed)
	{
		...
		{
			dpsmd = &pA->d3ddspmd[m];
			sprintf_s(buffer, "%d x %d", dpsmd->Width, dpsmd->Height);
		...
		for (UINT c = 0; c < pD->nNumCombo; ++c)
		{
			...
			{
				if (!ContainsString(m_hBACKFMT, D3DFormatToString(pC->fmtAdapter)))
					AddItem(m_hBACKFMT, D3DFormatToString(pC->fmtBackBuffer), &pC->fmtBackBuffer);
			}
		}
	}
...
HRESULT ZFXD3DEnum::EnumAdapters(void)
{
	...
	for (UINT nAdapter = 0; nAdapter < nNumAdapters; ++nAdapter)
	{
		...
		for (UINT nFmt = 0; nFmt < m_nNumFmt; ++nFmt)
		{
			...
			{
				// get number of available dislay mode
				m_pD3D->EnumAdapterModes(nAdapter, m_fmtAdapter[nFmt], nMode, &d3ddspmd);

				// dont't use less than we asked for
				if (d3ddspmd.Width < m_nMinWidth || d3ddspmd.Height < m_nMinHeigth || GetBits(d3ddspmd.Format) < m_nMinBits)
					continue;
				else
				{
		...
		// Sort data
		qsort(m_xAdapterInfo[m_dwNumAdapters].d3ddspmd, m_xAdapterInfo[m_dwNumAdapters].nNumModes, sizeof(D3DDISPLAYMODE), SortModesCallback);
...
HRESULT ZFXD3DEnum::EnumCombos(ZFXDEVICEINFO& xDev)
{
	...
	{
		// all allowed backbuffer formats
		for (UINT nFmt_B = 0; nFmt_B < g_nFormats_B; ++nFmt_B)
		{
			...
			for (UINT n = 0; n < 2; ++n)
			{
				...
				// at this point got a vaild combo supported by system
				// need to find depth/stencil fmt and best vertex processing type
				...
				if ((xDev.d3dCaps.DevCaps & D3DDEVCAPS_PUREDEVICE) != 0)
				{
					// 1. case	: pure device
					if (((xDev.d3dCaps.DevCaps & D3DDEVCAPS_PUREDEVICE) != 0)
						&& (ConfirmDevice(&xDev.d3dCaps, D3DCREATE_PUREDEVICE, pCombo->fmtBackBuffer) == ZFX_OK))
					{
						pCombo->dwBehavior = D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_PUREDEVICE;
					}

					// 2. case	: hardware device
					else if (ConfirmDevice(&xDev.d3dCaps, D3DCREATE_HARDWARE_VERTEXPROCESSING, pCombo->fmtBackBuffer) == ZFX_OK)
						pCombo->dwBehavior = D3DCREATE_HARDWARE_VERTEXPROCESSING;

					// 3. case	: mixed sw/hw
					else if (ConfirmDevice(&xDev.d3dCaps, D3DCREATE_MIXED_VERTEXPROCESSING, pCombo->fmtBackBuffer) == ZFX_OK)
						pCombo->dwBehavior = D3DCREATE_MIXED_VERTEXPROCESSING;
				}	// if [HW]
			
				// 4. case	: must be sw processing
				else {
					if (ConfirmDevice(&xDev.d3dCaps, D3DCREATE_SOFTWARE_VERTEXPROCESSING, pCombo->fmtBackBuffer) == ZFX_OK)
...
HRESULT ZFXD3DEnum::ConfirmDevice(D3DCAPS9* pCaps, DWORD dwBehavior, D3DFORMAT fmtBackBuffer)
{
	if ((dwBehavior & D3DCREATE_HARDWARE_VERTEXPROCESSING) || (dwBehavior & D3DCREATE_MIXED_VERTEXPROCESSING))
	{
		...
		{
			fprintf(m_pLog, "[XFXD3D_ENUM] error: no alphablending from texture \n");
			return ZFX_FAIL;
		}
...
const char* ZFXD3DEnum::D3DDevTypeToString(D3DDEVTYPE devType)
{
	switch (devType)
	{
...
const char* ZFXD3DEnum::D3DFormatToString(D3DFORMAT format)
{
	switch (format)
	{
...
const char* ZFXD3DEnum::BehaviorTypeToString(DWORD vpt)
{
	switch (vpt)
	{
